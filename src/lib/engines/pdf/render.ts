...
import { extractLines, extractPlainText, withDocument, type PdfDocument } from "./pdfjs";
...
export async function renderFirstPage(
  ...
  return await withDocument({ name: "preview.pdf", bytes }, async (document) => {
...
export const pdfToJpg: FileOp = async (files, options, onProgress) => {
  ...
  return await withDocument(files[0], async (document) => {
...
export const compressPdf: FileOp = async (files, options, onProgress) => {
  ...
  onProgress?.(0, "Starting the PDF engine…");
  return await withDocument(files[0], async (source) => {
    const target = await PDFDocument.create();
...
export const comparePdf: FileOp = async (files, options, onProgress) => {
  ...
  onProgress?.(0, "Starting the PDF engine…");
  // Two documents, two scopes: the second is opened inside the first so
  // that a failure reading the right-hand file still closes the left.
  return await withDocument(left, async (leftDoc) =>
    withDocument(right, async (rightDoc) => {

      async function readAll(document: PdfDocument, label: string) {
        const parts: string[] = [];
        for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber++) {
